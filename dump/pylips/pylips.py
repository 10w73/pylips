...
class Pylips:
        ...
        if (len(sys.argv)==1 or (len(sys.argv)==3 and sys.argv[1] == "--config")) and self.config["DEFAULT"]["mqtt_listen"] == "True":
            if len(self.config["MQTT"]["host"])>0:
                ...
                if self.config["DEFAULT"]["mqtt_update"] == "True":
                    ...
                    self.last_status = {"powerstate": None, "ambilight": False, "ambihue": False, "ambi_brightness": False}
    ...
    def get(self, path, verbose=True, err_count=0):
        while err_count < int(self.config["DEFAULT"]["num_retries"]):
            ...
            if self.config["DEFAULT"]["mqtt_listen"].lower()=="true":
                self.mqtt_update_status({"powerstate": "Off", "ambilight": False, "ambihue": False, "ambi_brightness": False})
    ...
    def mqtt_callback(self, path):
        if "ambilight" or "ambihue" or "ambi_brightness" in path:
            ...
            self.mqtt_update_ambilight_brightness_status()
    ...
    def mqtt_update_ambihue(self):
        ...
        if ambihue_status is not None and ambihue_status[0]=='{':
            print(str(ambihue_status) + "ambihue_status")
    ...
    def mqtt_update_ambilight_brightness_status(self):
        brightness_status = self.run_command("ambilight_brightness_status",None,self.verbose, False)
        ...
        while True:
            if self.mqtt_update_powerstate():
                ...
                self.mqtt.publish(str(self.config["MQTT"]["topic_status"]),
                                  json.dumps({"powerstate": "Off", "ambilight": False, "ambihue": False, "ambi_brightness": False}), retain=False)
